...
int main(int ac, char **av)
{
    ...
    if (myRank == 0)
    {
        ...
        if (processes > 1)
        {
            ...
            if (!report.good())
            {
                ...
                cerr << "*  Aby go utworzyÄ naleĹźy uruchomiÄ program z jednym procesem *" << endl;
                cerr << "***************************************************************" << endl;
                stopProgram = 1;
            }
            else
            {
                cout << "Znaleziono plik serial.txt - zostanie uĹźyty do testu." << endl;
            }
        }
    }

    MPI_Bcast(&stopProgram, 1, MPI_INT, 0, MPI_COMM_WORLD); // rozesĹanie informacji o wyĹÄczeniu programu
    ...
    else
    {
        ...
        if (myRank == 0)
        {
            ...
            cout << "Czas obliczen rĂłwnolegĹych  " << obliczeniaRownolegle << endl;
